    ...
    for examples in example_list:
        ...
        for example in examples:
            ...
            example_str += (
                ...
                + "\n\nReasoning: "
                + example["reasoning"].rstrip("\n")
                + "\n\nAnswer: "
                + example["answer"].rstrip("\n")
                + "\n\n"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_shots", type=int, default=2, help="Number of shots/examples to use"
    )
    args = parser.parse_args()
    num_shots = args.num_shots

    dataset_math_test_full = load_math500(split="test")
    ...
    dataset_math_test = [
        instance
        for instance in dataset_math_test_full
        if instance["id"] not in question_ids_solved_already
    ]

    dataset_math_train = load_reinforce(
        "../src/synth_data/cot_synth_data_70b_int4_50.json",
        len(dataset_math_test),
        num_shots,
        num_shots,
    )
